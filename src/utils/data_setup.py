...
def preprocess_fraud(file_path):
    """
    Preprocesses the DataFrame by scaling features, removing highly correlated ones, and applying SMOTE for class balancing.
    """
    ...
    df_filtered = df_var.drop(columns=columns_to_drop, axis=1)
...
def load_datasets(num_clients: int, batch_size: int, resize: int, seed: int, num_workers: int, splitter=10,
    ...
    list_transforms = [transforms.ToTensor(), transforms.Normalize(**NORMALIZE_DICT[dataset])] if dataset not in ["MMF", "DNA", "hiv", "Fraud"] else None
    print(dataset)
    ...
    if dataset == "cifar":
        ...
        trainset, testset = preprocess_fraud(data_path + dataset + '/fraud_detection_bank_dataset.csv')
    ...
    if dataset == "DNA":
        ...
        print("The training set is created for the classes: ('Non-Fraud', 'Fraud')")
    elif dataset == "MMF":
    ...
    elif data_path_val and dataset not in ["DNA", "hiv", "Fraud"]:
