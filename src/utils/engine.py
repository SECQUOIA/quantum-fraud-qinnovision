...
def test_fraud(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, loss_fn: Union[torch.nn.Module, Tuple],
         device: torch.device):
    """Tests a PyTorch model for a single epoch.
    ...
    with torch.inference_mode():
        ...
        for images, labels in dataloader:
            ...
            y_true.extend(labels)  # Save Truth            
            preds = np.round(output.squeeze().detach().cpu().numpy()).astype(int)
            y_pred.extend(preds)  # Save Prediction
            acc = (preds == labels).mean()
            test_acc += acc            
...
def train_step_fraud(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, loss_fn: Union[torch.nn.Module, Tuple],
               optimizer: torch.optim.Optimizer, device: torch.device) -> Tuple[float, float]:
    """Trains a PyTorch model for a single epoch.
    ...
    for batch, (images, labels) in enumerate(dataloader):
        ...
        # 2. Forward pass
        decoded, output = model(images)
        output = output.squeeze()
        
        # 3. Calculate  and accumulate loss    
        loss_reconstruction = loss_fn(decoded, images)
        loss_classification = nn.BCELoss()(output, labels)
        ...
        # Calculate and accumulate accuracy metric across all batches
        preds = np.round(output.detach().cpu().numpy()).astype(int)
        all_preds.extend(preds)  # Collect all predictions
        all_labels.extend(labels.cpu().numpy())  # Collect all true labels

    # Adjust metrics to get average loss and accuracy per batch
    train_loss = train_loss / len(dataloader)
    train_acc = accuracy_score(all_labels, all_preds) * 100  # In percentage
...
def train(model: torch.nn.Module, train_dataloader: torch.utils.data.DataLoader,
    ...
    for epoch in tqdm(range(epochs), colour="BLUE"):
        ...
        if task == "Multimodal" and isinstance(loss_fn, Tuple):
            ...
            train_step_fn = train_step_fraud
            test_fn = test_fraud
        else:
