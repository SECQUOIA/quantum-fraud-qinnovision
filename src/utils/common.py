...
def save_b_roc(targets, y_proba, path):
    """
    Saves the ROC curve for a binary classification task.
    
    Parameters:
    - targets (array-like): True binary labels (0 or 1).
    - y_proba (array-like): Predicted probabilities for the positive class.
    - path (str): File path to save the ROC plot (e.g., 'roc_curve.png').
    """
    # Calculate the ROC curve
    fpr, tpr, _ = roc_curve(targets, y_proba)
    roc_auc = auc(fpr, tpr)
